...
My analyses will attempt to reproduce the regression from the original paper. I will also calculate the Pearson correlation for the original data and the new data set. I may also plot their data and create a regression table in the final paper.

**Key analysis of interest** 

Key confirmatory analysis will be "a regression analysis predicting comparative purchase happiness from social class," as stated in original publication, which "revealed that social class positively predicted happiness." My analytic approach should create a similar figure (and correlation) to that in the original article. My current analytic plan does not include standardizing the beta. Both the IV (SES measure) and DV (subjective happiness rating) require reverse coding. 
...
Although not an appropriate analysis to replicate as a key findnig, I intend to also examine where comparative purchase happiness differs significantly from zero (the midpoint of the scale) using confidence intervals for the fitted values of the regression line.
